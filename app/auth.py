...
SECRET_KEY = os.getenv("SECRET_KEY", "impactbridge-secret-key")
...
def require_volunteer(current_user: User = Depends(get_current_user)) -> User:
    if current_user.role not in [UserRole.volunteer, UserRole.coordinator]:
...
def require_donor(current_user: User = Depends(get_current_user)) -> User:
    if current_user.role not in [UserRole.donor, UserRole.coordinator]:
